const createCommentsElements = (rawCommentLog) => {
  ...
  commentsWithTag = commentLog
    .map((comments) => {
      const { date, time, name, comment } = JSON.parse(comments.trim());
      return `<section class="user-comment"><p class="time-stamp">${date},${time}</p> <p class="username">${name}</p><p>${comment}</p></section>`;
    })
    .reverse()
    .join("");

  return `${commentsWithTag}`;
};
